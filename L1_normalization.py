...
for a in range(iter_num):
    ...
    for i in range(paranum):
        ...
        grad[i] += 2 * lam * W_hist[i]  #Adding the L1 regularization term
    W += (alpha / 8704) * grad #Updating the weights using the grad vlaue evaluated and printing loss function and weights
    W_hist = copy.deepcopy(W) #copying the values of weights into W_hist and taking their absolute values
    for e in W_hist:
        e = abs(e)
